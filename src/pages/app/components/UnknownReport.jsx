...
export default function UnknownReport({ onClose, goBack }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const storageRef = ref(
        storage,
        `unknown_status/${user.uid}/${Date.now()}_${photo.name}`
      );
    ...
    try {
      await addDoc(collection(db, "unknown_status"), reportData);
  ...
  return (
    <div>
      ...
      <h1 className="text-center font-semibold text-lg text-[#2e7d32]">Report Animal Sighting</h1>
